...
public class CoreGreedyAlgorithm extends Distance
{
    public int K=1;                                   //No of #Ants
    public int I=1;                                   //No of #Cycles
    ...
    void getGreedyOptimization()throws Exception
    {
        ...
        int  routeLocation;                              // to track the route counter it always points to #currentStop
            ...
            currentStop=0;
                ...
                while(Stops<(N)){
                       ...
                       nextStop=getNextStopByGreedy(currentStop);     // find next stop by diversificaion
...
//                 display(Visited);
    ...
    public int getNextStopByGreedy(int i)        // i= current stop
    {
        int S=-1;
        double arg_max=0;
        ...
        for(int j=1; j<N; j++)
        {
            if(Visited[j]!=1)
            {
                double  temp= (float) ( Tau[i][j]  *  Neta[i][j] );    //T[i][j]*(N[i][j])^B
